...
<template>
  <BCard class="h-100 mb-2" :key="game.identifier" :bg-variant="game.status===CluedoGames.Status.WAITING ? 'light': 'lightgray'">
    ...
    <BContainer>
      <gamer-description id="list-game" v-for="gamer in game.gamers" :gamer="gamer" :key="gamer.identifier" :online="game.status === CluedoGames.Status.WAITING" />
    </BContainer>
    ...
    <template #footer v-if="game.status===CluedoGames.Status.WAITING">
